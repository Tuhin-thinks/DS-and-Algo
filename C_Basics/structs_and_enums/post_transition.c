...
void print_all_packages(town t) {
    for (int i=0; i< t.offices_count; i++){  // iterate on post-offices
        printf("\t%d\n", i);
        for(int j=0; j<t.offices[i].packages_count; j++){  // iterate on package of each post office
...
town town_with_most_packages(town* towns, int towns_count) {
    ...
    for(int i=0; i<towns_count; i++){
        ...
        if (sum > max_so_far){
            max_package_town = towns[i];
            max_so_far = sum;
            sum = 0;
        }
    }

    return max_package_town;
}
